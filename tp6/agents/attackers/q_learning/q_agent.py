...
class QAgent(BaseAgent):
    ...
    def __init__(self, host, port, role="Attacker", alpha=0.1, gamma=0.6, epsilon_start=0.9, epsilon_end=0.1, epsilon_max_episodes=5000, apm_limit:int=None, randomize_topology: bool = False) -> None:
        ...
        self._randomize_topology = randomize_topology
        if self._apm_limit:
    ...
    @property
    def randomize_topology(self) -> bool:
        return self._randomize_topology
    ...
    def max_action_q(self, observation:Observation) -> float:
        ...
        for action in valid_actions:
            q_value = self.q_values.get((state_id, action), 0)
    ...
    def play_game(self, observation, episode_num, testing=False):
        ...
        while not observation.end:
            ...
            if not testing:
                ...
                current_q = self.q_values.get((state_id, action), 0)
                max_next_q = self.max_action_q(observation)
                new_q = current_q + self.alpha * (observation.reward + self.gamma * max_next_q - current_q)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--randomize_topology", help="Randomize topology between episodes", action="store_true", default=False)
    parser.add_argument("--use_wandb", help="Enable logging to Weights & Biases.", action="store_true", default=False)
    ...
    # Create agent
    agent = QAgent(args.host, args.port, alpha=args.alpha, gamma=args.gamma, epsilon_start=args.epsilon_start, epsilon_end=args.epsilon_end, epsilon_max_episodes=args.epsilon_max_episodes, apm_limit=args.apm, randomize_topology=args.randomize_topology)
    ...
    try:
        if use_wandb:
            ...
            agent._logger.info("Weights & Biases logging disabled by configuration.")
        ...
        if use_wandb:
            ...
            if not path.exists(args.env_conf):
                agent._logger.warning(f"Environment config file not found: {args.env_conf}")
        ...
        for episode in range(1, args.episodes + 1):
                if not early_stop:
                    ...
                    # Reset the game
                    observation = agent.request_game_reset(randomize_topology=agent.randomize_topology)
                    ...
                    if episode % args.test_each == 0 and episode != 0:
                        ...
                        if use_wandb:
                            ...
                            for test_episode in range(1, args.test_for + 1):
                                ...
                                # Reset the game
                                test_observation = agent.request_game_reset(randomize_topology=agent.randomize_topology)
